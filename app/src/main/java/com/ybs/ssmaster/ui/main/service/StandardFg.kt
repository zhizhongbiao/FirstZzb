...
import com.ybs.reslib.databinding.FgStandardBinding
...
class StandardFg : AppBaseFg<FgStandardBinding, MainVm, MainAct>() {

    companion object {
        fun newInst(url: String) = StandardFg().apply {
    ...
    override fun getLayoutBinding(
        ...
        container: ViewGroup?
    ) = FgStandardBinding.inflate(inflater, container, false)
    ...
    private fun handleService(bundle: Bundle) =
        bundle.getParcelable<QueryRestaurantCommResp>(KEY_QRCODE_DATA)?.apply {
            ...
            val tts = TtsUtil.getTts(templateConfig.displayText)
            binding?.tvContent?.text = tts

            loadImg(
                templateConfig.standardImage,
                binding?.iv!!,
                this@StandardFg,
                R.drawable.ic_image_holder
            )
        }
