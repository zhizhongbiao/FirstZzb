...
//         dialog?. window?.clearFlags(WindowManager.LayoutParams.FLAG_NOT_FOCUSABLE)
    ...
    protected open fun getW()=WindowManager.LayoutParams.MATCH_PARENT
    protected open fun getH()=WindowManager.LayoutParams.MATCH_PARENT

    override fun onKey(dialog: DialogInterface?, keyCode: Int, event: KeyEvent?) = false


    open fun getFgTag() = this.javaClass.simpleName+this.toString()


    fun showDlg(manager: FragmentManager, tag: String = getFgTag()) = this.apply {
        if (isResumed || isAdded || hasExist(manager, tag)) return this@BaseDialogFg
        e("show dlg fg tag  = $tag")
        show(manager, tag)
    }


    protected open fun hasExist(manager: FragmentManager, tag: String) =
        manager.findFragmentByTag(tag) != null

    override fun dismiss() {
        if (!isResumed) return
        e("dismiss dlg fg tag = ${getFgTag()}")
        super.dismiss()
    }
